...
class ExtWebElement(WebElement):
    ...
    def clear_textfield_value(self):
        text = self.get_value()
        i = 0
        while i < len(text):
            i += 1
    ...
    def get_elements_by_attribute(self, attribute, value):
        return (ExtWebElement(e) for e in self.find_elements_by_css_selector("*["+attribute+"='"+value+"']"))

    def get_element_by_attribute(self, attribute, value):
        return self.get_elements_by_attribute(attribute, value)[0]
    ...
    def find_elements_by_tag(self, name):
        return (ExtWebElement(e) for e in self.find_elements_by_tag_name(name))

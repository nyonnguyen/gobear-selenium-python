...
class GoBearCore():
    ...
    def __init__(self, browser):
        ...
        self.driver = browser.browser_driver
        self.wait = browser.wait
        self.element_finder = FindElement(self)

    def get_element(self, locator, tag=None):
        _locator = GBUtilies.extract_locator(locator)
        if GB_ATTRIBUTE_NAME in _locator[0].lower():
            return ExtWebElement(self.browser.find_element_by_attribute(GB_ATTRIBUTE_NAME, _locator[1]))
        else:
            return ExtWebElement(self.element_finder.find_element(locator, tag))

    def get_elements(self, locator, tag=None):
        _locator = GBUtilies.extract_locator(locator)
        if GB_ATTRIBUTE_NAME in _locator[0].lower():
            return self.browser.find_elements_by_attribute(GB_ATTRIBUTE_NAME, _locator[1])
        else:
            return [ExtWebElement(e) for e in self.element_finder.find_elements(locator, tag)]
    ...
    def wait_until_element_is_visible(self, locator, timeout=None, error=None):
        self.wait._until(
            lambda: self.is_element_visible(locator),
    ...
    def wait_until_element_is_enable(self, locator, timeout=None, error=None):
        self.wait._until(
            lambda: self.is_element_enable(locator),
            "Element %s is not visible <TIMEOUT>" % locator,
            timeout,
            error
        )
    ...
    def select_button_dropdown(self, locator, value):
        ...
        # show dropdown list
        control.get_element_by_tag('button').click()
...
################## DETAILS OPTIONS ####################
    ...
    def get_detail_options(self, locator):
        ...
        return sort_section.find_elements_by_tag('label')
...
#################### DATE PICKER ##########################
    ...
    def get_date_picker_switch(self):
        ...
        current_picker = self.driver.find_element_by_class(picker_class)
    ...
    def is_date_picker_days(self):
        try:
            return self.driver.find_element_by_class(GB_DATE_PICKER_DAYS_CLASS).is_displayed()
        except:
            return False

    def is_date_picker_months(self):
        try:
            return self.driver.find_element_by_class(GB_DATE_PICKER_MONTHS_CLASS).is_displayed()
        except:
            return False

    def is_date_picker_years(self):
        try:
            return self.driver.find_element_by_class(GB_DATE_PICKER_YEARS_CLASS).is_displayed()
        except:
            return False

    def get_date_picker_popup(self):
        return self.browser.find_element_by_class(GB_DATE_PICKER_MENU_CLASS)

    def is_date_picker_popup(self):
        return self.browser.is_any_element_contain_class(GB_DATE_PICKER_MENU_CLASS)
